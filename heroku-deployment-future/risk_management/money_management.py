...
class NonLeveragedLinear:
    ...
    def __init__(self, capital=300, zone=[0, 1]):
        self.capital = capital
        self.lower = zone[0]
        self.upper = zone[1]
        self.maxPos = self.capital/((self.upper+self.lower)/2)
...
class LeveragedLinear():
    ...
    def est_liq_price(self, mmr=0.03, side=None):
    ...
    def plot_linear(self, side, dec_price, dec_pos):
        ...
        print('%-23s %10.{0}f'.format(dec_price) % ('Est. liquidation price:', self.est_liq_price(side=side)[0]))
        print('-'*30)
